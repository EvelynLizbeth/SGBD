    ...
    void llamarABloque(string direccion_bloque,char indicadorLW) {
        ...
        if (pos != -1) {
            ...
            if (!poolSaturated()) {
                ...
                int pos = posicionPrimerFrameNoPinneadoClock();//posicionRecientementeMenosUsadoLRUMasOtrasEvaluaciones(); // Encontrar el frame menos recientemente usado con pin_count 0
                if (pos != -1) {
